...
func GetLectureSuggestion(id int) (LectureSuggestionData,error) {
	rows, err := utils.QuerySQLConn(DBNAME,"SELECT id,lecture_name,speaker_name,speaker_bio,lecture_desc,`from_nonprague`,preferences FROM lecture_suggestion WHERE id=?",id)
	if err != nil {
		log.Println(err.Error())
	}
	// HERE
...
func GetLectureSuggestions() ([]LectureSuggestionData,error) {
	rows, err := utils.QuerySQLConn(DBNAME,"SELECT id,lecture_name,speaker_name,speaker_bio,lecture_desc,`from_nonprague`,preferences FROM lecture_suggestion")
	if err != nil {
		log.Println(err.Error())
	}
	// HERE
