...
var signKey []byte = []byte("a4fJqmz/0x92Aea(aQQ12")
...
func JWTAuthMiddleware(next http.Handler) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if tokenString == "" {
			response := AuthResponse{"failure","no token provided"}
		...
		if (err != nil) {
			...
			response := AuthResponse{"failure","authentication token malformed"}
		...
		if token.Valid {
			response := AuthResponse{"success","authentication token valid"}
			...
			if ve.Errors&jwt.ValidationErrorMalformed != 0 {
				...
				response := AuthResponse{"failure","authentication token expired"}
				utils.WriteJSONResponse(w,response,500)
			} else if ve.Errors&jwt.ValidationErrorNotValidYet != 0 {
				response := AuthResponse{"failure","authentication token not in use yet"}
				utils.WriteJSONResponse(w,response,500)
			} else {
				response := AuthResponse{"failure","authentication token not in use anymore"}
				utils.WriteJSONResponse(w,response,500)
			}
		} else {
			response := AuthResponse{"failure","unexpected error while handling token"+err.Error()}
				utils.WriteJSONResponse(w,response,500)
		}
